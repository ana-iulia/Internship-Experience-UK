...
class VideoLibrary {
    ...
    List<Video> getVideosByTerm(String searchTerm) {
        List<Video> videos = new ArrayList<>();
        for (Video video : this.videos.values()) {
            if (Pattern.compile(Pattern.quote(searchTerm), Pattern.CASE_INSENSITIVE).matcher(video.getTitle()).find()) {
                videos.add(video);
            }
    ...
    List<Video> getVideosByTag(String searchTag) {
        List<Video> videos = new ArrayList<>();
        for (Video video : this.videos.values()) {
